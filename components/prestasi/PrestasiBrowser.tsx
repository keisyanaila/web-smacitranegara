...
import { KATEGORI_COLOR, type PrestasiItem } from '@/lib/prestasi';

export default function PrestasiBrowser({ items }: { items: PrestasiItem[] }) {
  const ALL_KATEGORI = ['Semua', ...Array.from(new Set(items.map((p) => p.kategori)))];
  const [aktif, setAktif] = useState('Semua');

  const filtered = aktif === 'Semua' ? items : items.filter((p) => p.kategori === aktif);

  const jumlahTingkat = (kw: string) =>
    items.filter((p) => p.tingkat.toLowerCase().includes(kw)).length;
    ...
    <main>
      ...
      <section style={{ background: '#023d17', padding: '22px 24px', borderBottom: '2px solid #C8973A' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', gap: 40, justifyContent: 'center', flexWrap: 'wrap' }}>
          {[
            { label: 'Total Prestasi', val: `${items.length}` },
            { label: 'Tingkat Nasional', val: `${jumlahTingkat('nasional')}` },
            { label: 'Tingkat Internasional', val: `${jumlahTingkat('internasional')}` },
            { label: 'Bidang', val: `${new Set(items.map((p) => p.kategori)).size}` },
          ].map((s) => (
